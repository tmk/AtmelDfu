...
const deviceInfo = [
  { name: "at90usb128x", vendorId: 1003, productId: 12283, flashSize: 131072, bootSize: 8192, eepromSize: 4096 },
  { name: "at90usb64x", vendorId: 1003, productId: 12281, flashSize: 65536, bootSize: 8192, eepromSize: 2048 },
  { name: "at90usb162", vendorId: 1003, productId: 12282, flashSize: 16384, bootSize: 4096, eepromSize: 512 },
  { name: "at90usb82", vendorId: 1003, productId: 12279, flashSize: 8192, bootSize: 4096, eepromSize: 512 },
  { name: "atmega32u6", vendorId: 1003, productId: 12274, flashSize: 32768, bootSize: 4096, eepromSize: 1024 },
  { name: "atmega32u4", vendorId: 1003, productId: 12276, flashSize: 32768, bootSize: 4096, eepromSize: 1024 },
  { name: "atmega32u2", vendorId: 1003, productId: 12272, flashSize: 32768, bootSize: 4096, eepromSize: 1024 },
  { name: "atmega16u4", vendorId: 1003, productId: 12275, flashSize: 16384, bootSize: 4096, eepromSize: 512 },
  { name: "atmega16u2", vendorId: 1003, productId: 12271, flashSize: 16384, bootSize: 4096, eepromSize: 512 },
  { name: "atmega8u2", vendorId: 1003, productId: 12270, flashSize: 8192, bootSize: 4096, eepromSize: 512 }
];
...
function parseStatus(data) {
  return {
    ...
    bwPollTimeOut: data.getUint8(1) | data.getUint8(2) << 8 | data.getUint8(3) << 16,
...
function selectPage(dev, page) {
  const cmd = new Uint8Array([6, 3, 0, 0]);
  cmd[3] = page;
...
async function writeMemory(dev, start, end, data, eeprom = false) {
...
async function readBlock(dev, start, end, eeprom = false) {
  ...
  let cmd = new Uint8Array([3, 0, 0, 0, 0, 0]);
  cmd[1] = eeprom ? 2 : 0;
  cmd[2] = start >> 8 & 255;
  cmd[3] = start & 255;
  cmd[4] = end >> 8 & 255;
...
async function readMemory(dev, start, end, eeprom = false) {
  if (start > end) throw new Error("Memory range error");
  let page = -1;
  let _start = start;
  let _end = 0;
...
function chipErase(dev) {
  const cmd = new Uint8Array([4, 0, 255]);
  return dev.controlTransferOut(
    {
      requestType: "class",
      recipient: "interface",
...
function blankCheck(dev, start, end) {
  const cmd = new Uint8Array([3, 1, 0, 0, 111, 255]);
  return dev.controlTransferOut(
    {
      requestType: "class",
      recipient: "interface",
...
async function launch(dev) {
  const cmd = new Uint8Array([4, 3, 0]);
  await dev.controlTransferOut(
    {
      requestType: "class",
      recipient: "interface",
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    async function checkBlank() {
      try {
        let d = deviceInfo.find((e) => e.productId === target.productId);
        if (d === void 0) {
          console.log("Unknown Device");
          return;
        }
        let result = await blankCheck(target, 0, d.flashSize - 1);
        console.log("byteWritten: " + result.bytesWritten);
        result = await getStatus(target);
        let stat = parseStatus(result.data);
        if (stat.bStatus == bStatus.OK) {
          console.log("Blank");
          message.value = "Blank";
        } else if (stat.bStatus === bStatus.errCHECK_ERASED) {
    ...
    function loadHex(text) {
      ...
      for (let index = 0; index < lines.length; index++) {
        ...
        if (line.length < 11) {
          throw new Error(`Invalid at line ${index + 1}`);
        }
        if (line.at(0) !== ":") {
          throw new Error(`Invalid at line ${index + 1}`);
        }
        ...
        if (checkSum !== (-sum & 255)) {
          throw new Error(`Checksum error at line ${index + 1}`);
        }
        bytes.shift();
    ...
    async function programFlash() {
      try {
        ...
        result = await writeMemory(target, 0, data.length - 1, data);
        message.value = "Verifing...";
        let mem = await readMemory(target, 0, data.length - 1);
    ...
    async function writeFlash() {
      try {
        ...
        let data = loadHex(text);
        message.value = "Writing Flash...";
        let result = await writeMemory(target, 0, data.length - 1, data);
        message.value = "Verifing...";
        let mem = await readMemory(target, 0, data.length - 1);
        for (let i = 0; i < mem.byteLength; i++) {
          if (mem[i] !== data[i]) {
    ...
    async function readFlash() {
      try {
        ...
        let mem = await readMemory(target, 0, end, false);
        message.value = "Done";
        let fl = document.querySelector("#file-link");
        let fd = document.querySelector("#file-download");
    ...
    async function clearEEPROM() {
      try {
        ...
        data.fill(255, 0, d.eepromSize);
        message.value = "Writing EEPROM...";
        let result = await writeMemory(target, 0, end, data, true);
        message.value = "Verifing...";
        let mem = await readMemory(target, 0, end, true);
        for (let i = 0; i < mem.byteLength; i++) {
          if (mem[i] !== data[i]) {
            console.log(hexStr(i, 4) + ": " + hexStr(mem[i]));
            message.value = `Failed to verify at ${hexStr(i, 4)}`;
    ...
    async function writeEEPROM() {
      try {
        ...
        let data = loadHex(text);
        message.value = "Writing EEPROM...";
        let result = await writeMemory(target, 0, data.length - 1, data, true);
        message.value = "Verifing...";
        let mem = await readMemory(target, 0, data.length - 1, true);
        for (let i = 0; i < mem.byteLength; i++) {
          if (mem[i] !== data[i]) {
            console.log(hexStr(i, 4) + ": " + hexStr(mem[i]));
            message.value = `Failed to verify at ${hexStr(i, 4)}`;
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
        createBaseVNode("h1", _hoisted_2, "Target: " + toDisplayString(device.value), 1),
        createBaseVNode("h3", null, [
          createTextVNode(" Device: " + toDisplayString(device.value) + " ", 1),
          createBaseVNode("button", {
            disabled: unref(isButtonDisabled),
            onClick: selectDevice
          }, "Select Device", 8, _hoisted_3)
        ]),
        createBaseVNode("h3", null, [
          createTextVNode(" Status: " + toDisplayString(status.value) + " ", 1),
          createBaseVNode("button", { onClick: checkStatus }, "Check Status")
        ]),
        createBaseVNode("h3", null, [
          createBaseVNode("button", { onClick: eraseChip }, "Erase Chip"),
          createBaseVNode("button", { onClick: checkBlank }, "Check Blank")
        ]),
        createBaseVNode("h3", null, [
          createBaseVNode("button", { onClick: programFlash }, "Program Flash"),
          createBaseVNode("button", { onClick: writeFlash }, "Write Flash"),
          createBaseVNode("button", { onClick: readFlash }, "Read Flash"),
        ...
        createBaseVNode("h3", null, [
          _cache[2] || (_cache[2] = createBaseVNode("label", { for: "avatar" }, "Firmware File:", -1)),
          createBaseVNode("input", _hoisted_4, null, 512)
        ]),
        createBaseVNode("h3", null, [
          createBaseVNode("button", { onClick: clearEEPROM }, "Clear EEPROM"),
          createBaseVNode("button", { onClick: writeEEPROM }, "Write EEPROM"),
          createBaseVNode("button", { onClick: readEEPROM }, "Read EEPROM"),
        ...
        _cache[5] || (_cache[5] = createBaseVNode("h3", null, [
          createBaseVNode("label", { for: "avatar" }, "EEPROM File:"),
        ...
        createBaseVNode("h3", null, [
          createBaseVNode("button", { onClick: startApp }, "Start App"),
          createBaseVNode("button", { onClick: recoverError }, "Recover Error")
        ]),
        createBaseVNode("h3", null, " Message: " + toDisplayString(message.value), 1)
      ]);
    };
  }
};
const AtmelDfu = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-d81a573a"]]);
const _sfc_main = {
  __name: "App",
  setup(__props) {
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        _cache[0] || (_cache[0] = createBaseVNode("header", null, null, -1)),
        createBaseVNode("main", null, [
          createVNode(AtmelDfu, { d: "ATMega32U4" })
        ])
      ], 64);
    };
  }
};
const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-c91f3d67"]]);
